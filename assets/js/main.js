...
// Carousel Implementation (only if carousel elements exist on the page)
const promoSection = document.getElementById('promo');
if (promoSection) {
  const track = promoSection.querySelector('.carousel-track');
  const slides = promoSection.querySelectorAll('.carousel-slide') ? Array.from(promoSection.querySelectorAll('.carousel-slide')) : [];
  ...
  const dots = promoSection.querySelectorAll('.indicator-dot') ? Array.from(promoSection.querySelectorAll('.indicator-dot')) : [];
  const status = promoSection.querySelector('.carousel-status');
  const viewport = promoSection.querySelector('.carousel-viewport');

  // Check if carousel elements exist
  if (track && slides.length > 0 && nextBtn && prevBtn && playPauseBtn && viewport) {
    let currentIndex = 0;
