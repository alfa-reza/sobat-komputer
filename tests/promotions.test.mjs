...
test("builds slide rules for 0, 1, 4, and N promotions", () => {
  ...
  for (const count of [1, 4, 7]) {
    const views = buildPromotionSlides(
      Array.from({ length: count }, (_, index) =>
        promotion(`promo-${index + 1}`),
      ),
    );
    assert.equal(views.length, count);
    assert.deepEqual(
      views.map(({ position }) => position),
    ...
    assert.ok(
      views
        .slice(1)
        .every(({ hidden, loading }) => hidden && loading === "lazy"),
    );
  }
});
...
test("preloads every candidate and preserves API order", async () => {
  ...
  const loaded = [];
  const result = await preloadPromotionCandidates(input, async (src) =>
    loaded.push(src),
  );
  assert.deepEqual(
    loaded,
  ...
  assert.deepEqual(
    await preloadPromotionCandidates([], async () =>
...
test("one broken image rejects the complete candidate", async () => {
  ...
  await assert.rejects(
    preloadPromotionCandidates(input, async (src) => {
      if (src.includes("broken")) throw new Error("broken image");
    }),
    /broken image/,
  );
  await assert.rejects(
    preloadPromotionCandidates([promotion("good"), null], async () => {}),
    /Invalid promotion media/,
  );
});
