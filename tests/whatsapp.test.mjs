...
test("builds WhatsApp URLs according to intent", () => {
  ...
  assert.equal(
    promo.message,
    "Halo New Sobat Komputer, saya mau tanya detail promo terbaru.",
  );
  ...
  assert.equal(
    service.message,
    "Halo New Sobat Komputer, saya ingin menanyakan layanan servis.",
  );
  ...
  assert.equal(
    location.message,
    "Halo New Sobat Komputer, saya ingin menanyakan lokasi toko.",
  );
});
