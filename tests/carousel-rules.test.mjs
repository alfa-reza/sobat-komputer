...
} from "../assets/js/public/carousel.mjs";

test("carousel rules support 0, 1, 3, 4, 7, and 15 slides", () => {
  assert.deepEqual(getCarouselRules(0), {
    count: 0,
    hasSlides: false,
    isInteractive: false,
  });
  assert.deepEqual(getCarouselRules(1), {
    count: 1,
    hasSlides: true,
    isInteractive: false,
  });
  ...
  for (const count of [3, 4, 7, 15]) {
    assert.deepEqual(getCarouselRules(count), {
      count,
      hasSlides: true,
      isInteractive: true,
    });
  ...
  assert.deepEqual(getCarouselRules(2.5), {
    count: 0,
    hasSlides: false,
    isInteractive: false,
  });
