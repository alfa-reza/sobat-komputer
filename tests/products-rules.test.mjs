...
test("normalizes products without mutating input", () => {
  const input = [
    ...
    product({
      id: "product-2",
      image: image("assets/images/product-2.webp", "Gambar produk 2"),
    }),
  ];
  const before = structuredClone(input);
  const normalized = normalizeProductCandidates(input);
  assert.deepEqual(
    normalized.map((p) => p.image.src),
    ["assets/images/product-1.webp", "assets/images/product-2.webp"],
  );
...
test("builds restricted views and routes to Admin 2 without reference code", () => {
  const normalized = normalizeProductCandidates([
    ...
    product({
      id: "product-2",
      image: image("assets/images/product-2.webp", "Gambar 2"),
    }),
  ]);
  const views = buildProductViews(normalized, {
    pageUrl: "https://example.com/produk.html?token=secret#private",
  });
...
test("preserves product order in generated views", () => {
  ...
  const views = buildProductViews(products, {
...
test("preloads every image and rejects the complete candidate on failure", async () => {
  const views = buildProductViews(
    normalizeProductCandidates([
      ...
      product({
        id: "second",
        image: image("assets/images/product-2.webp", "2"),
      }),
    ]),
    { pageUrl: "https://example.com/produk.html" },
  );
  ...
  const loaded = [];
  assert.equal(
    await preloadProductViews(views, async (src) => loaded.push(src)),
    views,
  );
  ...
  await assert.rejects(
    preloadProductViews(views, async (src) => {
  ...
  assert.deepEqual(
    await preloadProductViews([], async () =>
      assert.fail("loader should not run"),
    ),
    [],
  );
});
