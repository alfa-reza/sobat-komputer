...
test('facade exports exactly three asynchronous read functions', async () => {
  assert.deepEqual(Object.keys(api).sort(), ['getHero', 'getProducts', 'getPromotions']);
  for (const fn of Object.values(api)) assert.equal(typeof fn().then, 'function');
...
test('promotion schedule uses inclusive start and exclusive end', async () => {
  ...
  assert.equal((await adapter.getPromotions({ now: '2026-07-19T09:59:59.999Z' })).length, 0);
  assert.equal((await adapter.getPromotions({ now: '2026-07-19T10:00:00.000Z' })).length, 1);
  assert.equal((await adapter.getPromotions({ now: '2026-07-19T11:00:00.000Z' })).length, 0);
});
...
test('status filtering and deterministic row ordering are enforced', async () => {
  const adapter = createMockContentAdapter(content({
    promotions: [
      ...
      promotion({ id: 'b', sort_order: 1, created_at: '2026-01-02T00:00:00.000Z' }),
      promotion({ id: 'a', sort_order: 1, created_at: '2026-01-02T00:00:00.000Z' }),
      promotion({ id: 'older', sort_order: 1, created_at: '2026-01-01T00:00:00.000Z' }),
...
test('product limit is clamped and validated', async () => {
  ...
  await assert.rejects(adapter.getProducts({ limit: '1' }), { name: 'ContentApiError', code: 'VALIDATION_ERROR', message: 'Invalid content request.' });
});

test('invalid request and malformed source errors are safe', async () => {
  const adapter = createMockContentAdapter(null);
  await assert.rejects(api.getPromotions({ now: 'not-a-date' }), { name: 'ContentApiError', code: 'VALIDATION_ERROR', message: 'Invalid content request.' });
  await assert.rejects(adapter.getHero(), { name: 'ContentApiError', code: 'FETCH_FAILED', message: 'Content is temporarily unavailable.' });
  await assert.rejects(adapter.getProducts(), (error) => !JSON.stringify(error).includes('null'));
});
...
test('results are fresh and media paths remain safe and relative', async () => {
  ...
  assert.ok(second.every(({ src }) => !src.startsWith('/') && !src.includes('..') && !src.includes('\\') && !src.includes('://')));
});
