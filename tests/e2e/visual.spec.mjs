...
test.describe("Visual Snapshots", () => {
  for (const p of pages) {
    test(`Visual baseline for ${p.name}`, async ({
      page,
      browserName,
    }, testInfo) => {
      ...
      test.skip(
        ...
        "Visual tests only run on specific viewports",
      );
      ...
      if (p.name === "kontak") {
        await page.addStyleTag({
          content: "iframe { visibility: hidden !important; }",
        });
      }
      ...
      await expect(page).toHaveScreenshot(
        `${p.name}-${project.toLowerCase().replace(" ", "-")}.png`,
        {
          fullPage: true,
